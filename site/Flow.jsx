// Flow — step-by-step process with accordion FAQ
function Flow(){
  const steps = [
  ...
  return (
    <section id="flow" style={{padding:'clamp(60px,10vw,120px) 0',
      ...
      <div className="wrap">
        <div className="reveal" style={{marginBottom:40}}>
          ...
          <h2 className="section">
            作る、<br/>
            <span style={{background:'var(--blue)',color:'#fff',padding:'0 0.14em',
        ...
        <div style={{display:'grid',gap:16,
          ...
          {steps.map((s,i)=>(
            <div key={s.n} className="nb-card reveal" style={{
              ...
              <div style={{fontFamily:'"Archivo Black",sans-serif',fontSize:48,
                lineHeight:1,opacity:.9,letterSpacing:'-0.04em'}}>{s.n}</div>
              <div style={{fontSize:'clamp(18px,1.6vw,22px)',fontWeight:900,
                marginTop:14,letterSpacing:'-0.02em'}}>{s.title}</div>
              <div style={{fontSize:12,marginTop:4,opacity:.75,
                fontFamily:'"Space Mono",monospace'}}>/ {s.jp}</div>
              <p style={{margin:'14px 0 0',fontSize:13.5,lineHeight:1.75,opacity:.92}}>
                {s.body}
              </p>
            </div>
          ))}
        ...
        <div style={{marginTop:80,display:'grid',gridTemplateColumns:'1fr',gap:40}}>
          <div className="reveal">
            ...
            <h3 style={{margin:0,fontSize:'clamp(26px,3.5vw,38px)',fontWeight:900,
          ...
          <div className="acc reveal" style={{
            ...
            {faqs.map((f,i)=>(
              <details key={i} open={i===0}>
                <summary>
                  <span><span style={{opacity:.5,marginRight:10,
                    fontFamily:'"Space Mono",monospace',fontSize:14}}>Q{String(i+1).padStart(2,'0')}</span>
                    {f.q}</span>
